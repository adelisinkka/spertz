    ...
    (function () {
      ...
      function compressMelodyTimeline(melody) {
        const thresholdMs = 520;
        const compressed = [];
        melody.forEach(([timeMs, holdMs]) => {
          if (!compressed.length || timeMs - compressed[compressed.length - 1][0] >= thresholdMs) {
            compressed.push([timeMs, holdMs]);
            return;
          }
          const [prevTime, prevHold] = compressed[compressed.length - 1];
          if (holdMs > prevHold || (holdMs === prevHold && timeMs - prevTime > thresholdMs * 0.45)) {
            compressed[compressed.length - 1] = [timeMs, holdMs];
          }
        });
        return compressed;
      }
