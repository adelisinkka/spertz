    ...
    (function () {
      ...
      function buildHardLaneModeChart(songKey, laneCount = LANE_MODE_LANES, mode = "lanes") {
        ...
        function activeLongNotesAt(timeMs) {
          return notes.filter((note) => note.endTimeMs && note.endTimeMs > timeMs && note.timeMs < timeMs + 120);
        }
        ...
        function pickLane(timeMs, preferred, avoid = [], requiredSide = "") {
          let available = availableLanesAt(timeMs).filter((lane) => !avoid.includes(lane));
          if (requiredSide) {
        ...
        grouped.forEach((group, groupIndex) => {
          ...
          const desiredCount = (longLead || secondLong)
            ? 1
      ...
      async function startMusicSong(songKey, mode = "freeform", difficulty = "normal") {
        ...
        state.hitWindowMs = (mode === "lanes" || mode === "sevenlanes") ? 150 : 185;
