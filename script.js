    ...
    (function () {
      ...
      const statefulNoiseBufferCache = new WeakMap();
      ...
      function prepareSongData(songKey, mode, loaded) {
        const preparedKey = `${songKey}:${mode}`;
      ...
      async function preloadMusicGameData(state) {
        ...
        const tasks = [
          loadSongChart("fiu", "freeform").then((loaded) => prepareSongData("fiu", "freeform", loaded)),
          loadSongChart("fiu", "lanes").then((loaded) => prepareSongData("fiu", "lanes", loaded)),
      ...
      function createNoiseBuffer(ctx) {
        if (statefulNoiseBufferCache.has(ctx)) return statefulNoiseBufferCache.get(ctx);
        const buffer = ctx.createBuffer(1, Math.max(1, Math.floor(ctx.sampleRate * 0.18)), ctx.sampleRate);
        const data = buffer.getChannelData(0);
        for (let i = 0; i < data.length; i += 1) data[i] = Math.random() * 2 - 1;
        statefulNoiseBufferCache.set(ctx, buffer);
        return buffer;
      }
      ...
      function playHitSound(state, note = null, judgment = "great") {
        ...
        function pulse(type, frequency, duration, gainStart, sweepTo) {
        ...
        if (isBonus) {
          pulse("triangle", judgment === "perfect" ? 920 : 760, 0.18, 0.18, 420);
          pulse("sine", 1280, 0.12, 0.08, 640);
          return;
        }

        if (kind === "drag" || kind === "hold") {
          pulse("triangle", 240, 0.14, 0.16, 92);
          pulse("sine", 420, 0.09, 0.06, 210);
          return;
        }
        ...
        if (kind === "flick") {
          ...
          src.start(now);
          src.stop(now + 0.1);
          pulse("sine", 680, 0.08, 0.035, 340);
          return;
        }

        pulse("sine", judgment === "perfect" ? 150 : 132, 0.16, 0.18, 48);
        pulse("triangle", 720, 0.03, 0.035, 180);
      }
      ...
      function createLaneNoteElement(state, note) {
        ...
        root.className = `music-game-lane-note type-${note.type}${note.bonus ? " is-bonus" : ""}`;
        root.innerHTML = `
      ...
      async function startMusicSong(songKey, mode = "freeform") {
        ...
        try {
          ...
          const preparedKey = `${songKey}:${mode}`;
          const loaded = preparedSongDataCache.get(preparedKey) || prepareSongData(songKey, mode, await loadSongChart(songKey, mode));
