    ...
    (function () {
      ...
      function compressMelodyTimeline(melody) {
        ...
        while (index < melody.length) {
          ...
          const picked = bucket.reduce((best, note) => {
            const bestScore = (best[1] > 0 ? 1200 : 0) + best[1] + ((best[0] - startTime) > 180 ? 60 : 0);
            const noteScore = (note[1] > 0 ? 1200 : 0) + note[1] + ((note[0] - startTime) > 180 ? 60 : 0);
        ...
        compressed.forEach((note, noteIndex) => {
          ...
          if (!next) return;
          if (next[0] - note[0] <= 1450) return;

          const between = melody.filter((candidate) => candidate[0] > note[0] + 280 && candidate[0] < next[0] - 280);
          if (!between.length) return;
          const bridge = between.reduce((best, candidate) => {
            const midpoint = note[0] + (next[0] - note[0]) * 0.5;
            const bestScore = (best[1] > 0 ? 900 : 0) + best[1] - Math.abs(best[0] - midpoint) * 0.2;
            const candidateScore = (candidate[1] > 0 ? 900 : 0) + candidate[1] - Math.abs(candidate[0] - midpoint) * 0.2;
      ...
      async function loadSongChart(songKey) {
        if (musicChartCache.has(songKey)) return musicChartCache.get(songKey);
        const song = MUSIC_SONGS[songKey];
        if (!song) throw new Error("Unknown song");
        const builtin = materializeBuiltinMusicChart(songKey) || BUILTIN_MUSIC_CHARTS[songKey];
        if (builtin) {
          musicChartCache.set(songKey, builtin);
          return builtin;
        }
        ...
        musicChartCache.set(songKey, parsed);
        return parsed;
      }
      ...
      async function startMusicSong(songKey) {
        if (!musicGameState) return;
        ...
        try {
          ...
          const audioStartPerf = performance.now();
          const playPromise = state.audio.play();
          const loaded = await loadSongChart(songKey);
          ...
          state.songStartPerf = audioStartPerf;
      ...
      function buildMusicGameUi() {
        ...
        songList.addEventListener("click", (event) => {
          ...
          startMusicSong(target.getAttribute("data-song"));
        });
