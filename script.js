    ...
    (function () {
      ...
      const validCodes = new Map([
        ...
        ["fiufiuzufiutazufiu", { type: "numberboom", label: "800000088" }],
        ["piuliul iriu", { type: "piuliul", label: "piuliul iriu" }],
        ["biupi", { type: "biupi", label: "biupi" }],
        ["lapi", { type: "lapi", label: "lapi" }],
        ["biupibiupi", { type: "biupibiupi", label: "biupibiupi" }],
        ["lapilapi", { type: "lapilapi", label: "lapilapi" }],
      ...
      function clearEffect() {
        overlay.classList.remove("effect-burst", "effect-rain", "effect-halo", "effect-violet", "effect-laser", "effect-thunder", "effect-elements", "effect-iceworld", "effect-midnight", "effect-fiu", "effect-spiretz", "effect-fiush", "effect-hearts", "effect-polarbear", "effect-mama", "effect-numberboom", "effect-piuliul", "effect-biupi", "effect-lapi", "effect-biupibiupi", "effect-lapilapi");
      ...
      const FREEFORM_PATHS = [
        [[0.16, 0.7], [0.26, 0.61], [0.37, 0.51], [0.49, 0.43], [0.61, 0.37], [0.73, 0.33], [0.84, 0.31]],
        [[0.16, 0.34], [0.28, 0.28], [0.41, 0.24], [0.55, 0.27], [0.67, 0.36], [0.76, 0.5], [0.82, 0.64]],
        [[0.18, 0.55], [0.29, 0.44], [0.4, 0.36], [0.5, 0.33], [0.6, 0.36], [0.71, 0.44], [0.82, 0.55]],
        [[0.18, 0.24], [0.29, 0.24], [0.4, 0.24], [0.51, 0.24], [0.62, 0.24], [0.73, 0.24], [0.84, 0.24]],
        [[0.18, 0.72], [0.29, 0.72], [0.4, 0.72], [0.51, 0.72], [0.62, 0.72], [0.73, 0.72], [0.84, 0.72]],
        [[0.2, 0.2], [0.2, 0.3], [0.2, 0.42], [0.2, 0.54], [0.2, 0.64], [0.2, 0.74]],
        [[0.8, 0.2], [0.8, 0.3], [0.8, 0.42], [0.8, 0.54], [0.8, 0.64], [0.8, 0.74]],
        [[0.18, 0.26], [0.3, 0.36], [0.42, 0.46], [0.54, 0.56], [0.66, 0.66], [0.78, 0.76]],
        [[0.18, 0.76], [0.3, 0.66], [0.42, 0.56], [0.54, 0.46], [0.66, 0.36], [0.78, 0.26]],
      ];
      const LANE_MODE_LANES = 4;
      const SEVEN_LANE_MODE_LANES = 7;

      function compressMelodyTimeline(melody, songKey = "") {
        const windowMs = songKey === "recordplayer" ? 320 : 650;
        const compressed = [];
        let index = 0;
        ...
        compressed.forEach((note, noteIndex) => {
          ...
          if (!next) return;
          if (next[0] - note[0] <= (songKey === "recordplayer" ? 700 : 1500)) return;

          const edgePadding = songKey === "recordplayer" ? 110 : 260;
          const between = melody.filter((candidate) => candidate[0] > note[0] + edgePadding && candidate[0] < next[0] - edgePadding);
      ...
      function compressMelodyTimelineHard(melody, songKey = "") {
        const compressed = [];
        let lastTime = -Infinity;
        melody.forEach((note, index) => {
          const minGap = songKey === "recordplayer"
            ? (note[1] >= 700 ? 78 : note[1] >= 240 ? 68 : 84)
            : songKey === "palace"
              ? (note[1] >= 700 ? 86 : note[1] >= 240 ? 72 : 84)
              : (note[1] >= 700 ? 130 : note[1] >= 360 ? 110 : 150);
          const keepStride = songKey === "recordplayer" || songKey === "palace" ? 2 : 3;
          if (note[0] - lastTime < minGap && index % keepStride !== 0) return;
      ...
      function buildMusicChartLayout(melody, difficulty = "normal") {
        const activeWindowMs = 2600;
        const placed = [];
        let dragCount = 0;
        let consecutiveDrags = 0;
        const targetDragRatio = difficulty === "hard" ? 0.1 : 0.06;
        ...
        return melody.map(([timeMs, holdMs], index) => {
          ...
          const notesSoFar = Math.max(1, index + 1);
          const dragQuotaOpen = dragCount / notesSoFar < targetDragRatio;
          const farEnoughFromLast = !placed.length || timeMs - placed[placed.length - 1].timeMs >= (difficulty === "hard" ? 700 : 950);
          const longEnough = holdMs >= (difficulty === "hard" ? 900 : 1200);
          const canUseDrag = longEnough && dragQuotaOpen && farEnoughFromLast && consecutiveDrags < 1;
          const note = canUseDrag
            ? {
                id: `drag-${index}`,
                type: "drag",
                timeMs,
                endTimeMs: timeMs + Math.min(holdMs, difficulty === "hard" ? 1150 : 1000),
                x: startPoint.x,
                y: startPoint.y,
      ...
      function materializeBuiltinMusicChart(songKey, difficulty = "normal") {
        ...
        const compressedMelody = difficulty === "hard"
          ? compressMelodyTimelineHard(builtin.melody, songKey)
          : compressMelodyTimeline(builtin.melody, songKey);
        return {
          bpm: builtin.bpm,
          durationMs: builtin.durationMs,
          chart: buildMusicChartLayout(compressedMelody, difficulty),
        };
      }
      ...
      function buildLaneModeChart(songKey, timingOverride = null) {
        ...
        const groupWindowMs = songKey === "recordplayer" ? 120 : 210;
        const stallionBoost = songKey === "stallion";
        ...
        melody.forEach((entry) => {
          ...
          if (!group || entry[0] - group.anchor > groupWindowMs) {
        ...
        function activeCountAt(timeMs) {
          return notes.filter((note) => note.timeMs <= timeMs && (note.endTimeMs || note.timeMs + 150) > timeMs).length;
        }

        function activeLongNotesAt(timeMs) {
          return notes.filter((note) => note.endTimeMs && note.timeMs <= timeMs && note.endTimeMs > timeMs);
        }

        function remainingCapacityAt(timeMs) {
          return Math.max(0, 2 - activeCountAt(timeMs));
        }
        ...
        function blockedByActiveLong(lane, timeMs) {
          return activeLongNotesAt(timeMs).some((note) => {
            const span = getLaneNoteSpan(note);
            const noteSide = lane < span.min ? "left" : lane > span.max ? "right" : "";
            if (!noteSide) return false;
            if (note.type === "drag") {
              const dragHand = getLaneDragHand(note, notes);
              if (dragHand === "left" && noteSide === "left") return true;
              if (dragHand === "right" && noteSide === "right") return true;
            }
            const lockedSide = notes
              .filter((entry) => entry !== note && entry.timeMs <= timeMs && (entry.endTimeMs || entry.timeMs + 150) > timeMs)
              .map((entry) => {
                const otherSpan = getLaneNoteSpan(entry);
                if (otherSpan.max < span.min) return "left";
                if (otherSpan.min > span.max) return "right";
                return "";
              })
              .find((side) => side === "left" || side === "right");
            return Boolean(lockedSide && lockedSide !== noteSide);
          });
        }

        function dragHandForSpan(startLane, endLane) {
          return ((startLane + endLane) * 0.5) < (LANE_MODE_LANES - 1) * 0.5 ? "left" : "right";
        }

        function hasDragConflict(timeMs, endTimeMs, startLane, endLane) {
          return notes.some((note) => {
            if (note.type !== "drag" || !note.endTimeMs) return false;
            if (note.endTimeMs <= timeMs || note.timeMs >= endTimeMs) return false;
            const spanA = { min: Math.min(startLane, endLane), max: Math.max(startLane, endLane) };
            const spanB = { min: Math.min(note.lane, note.endLane ?? note.lane), max: Math.max(note.lane, note.endLane ?? note.lane) };
            const overlap = spanA.min <= spanB.max + 1 && spanB.min <= spanA.max + 1;
            const sameHand = dragHandForSpan(startLane, endLane) === dragHandForSpan(note.lane, note.endLane ?? note.lane);
            return overlap || sameHand;
          });
        }

        function noteTypeFor(timeMs, holdMs) {
          if (stallionBoost && holdMs >= 980) return emittedIndex % 6 === 2 ? "drag" : "hold";
          if (stallionBoost && holdMs >= 420) return emittedIndex % 7 === 3 ? "hold" : "tap";
          if (stallionBoost && emittedIndex % 9 === 4 && timeMs > 8000) return "flick";
          if (holdMs >= 900) return emittedIndex % 4 === 1 ? "drag" : "hold";
          if (holdMs >= 520) return emittedIndex % 5 === 2 ? "drag" : "hold";
          if (holdMs >= 220) return emittedIndex % 4 === 0 ? "hold" : "tap";
        ...
        function pushLaneNote(timeMs, holdMs, lane, paired = false) {
          ...
          if (type === "drag") {
            ...
            if (hasDragConflict(timeMs, endTimeMs, lane, endLane)) {
              const holdEndTimeMs = timeMs + Math.max(700, holdMs);
              notes.push({
                id: `lane-hold-${notes.length}`,
                type: "hold",
                timeMs,
                lane,
                endTimeMs: holdEndTimeMs,
              });
              reserveLane(lane, holdEndTimeMs);
              lastLane = lane;
              emittedIndex += 1;
              return;
            }
        ...
        grouped.forEach((group, groupIndex) => {
          ...
          const activeLongCount = activeLongNotesAt(ranked[0]?.[0] ?? 0).length;
          const shouldDouble = activeLongCount === 0 && ranked.length >= 2 && (stallionBoost ? groupIndex % 4 === 0 : groupIndex % 2 === 0);
          const targetCount = Math.min(shouldDouble ? 2 : 1, remainingCapacityAt(ranked[0]?.[0] ?? 0));
          const chosenEntries = ranked.slice(0, targetCount);
          const simultaneousTime = chosenEntries[0]?.[0];
          if (simultaneousTime == null) return;
          if (activeLongNotesAt(simultaneousTime).length >= 2) return;
          if (activeCountAt(simultaneousTime) >= 2) return;

          const available = availableLanesAt(simultaneousTime).filter((lane) => !blockedByActiveLong(lane, simultaneousTime));
          if (!available.length) return;

          const primaryPreferred = stallionBoost
            ? [0, 2, 3, 1, 3, 0, 2, 1][groupIndex % 8]
            : (lastLane <= 1 ? 2 : lastLane >= LANE_MODE_LANES - 2 ? 1 : lastLane + (groupIndex % 2 === 0 ? 1 : -1));
          ...
          const secondAvailable = availableLanesAt(simultaneousTime).filter((lane) => lane !== firstLane && !blockedByActiveLong(lane, simultaneousTime));
          if (!secondAvailable.length) return;
          const oppositePreferred = stallionBoost
            ? (firstLane <= 1 ? 3 : 0)
            : (firstLane < LANE_MODE_LANES / 2 ? LANE_MODE_LANES - 1 : 0);
        ...
        grouped.slice(1).forEach((group, index) => {
          ...
          if (currentTime - previousTime < (stallionBoost ? 900 : 1100)) return;
          const bridge = melody.find((entry) => entry[0] > previousTime + (stallionBoost ? 240 : 320) && entry[0] < currentTime - (stallionBoost ? 240 : 320) && entry[1] <= 520);
          if (!bridge) return;
          if (activeLongNotesAt(bridge[0]).length >= 2) return;
          if (remainingCapacityAt(bridge[0]) < 1) return;
          const available = availableLanesAt(bridge[0]).filter((lane) => !blockedByActiveLong(lane, bridge[0]));
          const bridgeLane = chooseLane(available, stallionBoost ? ((lastLane + 1) % LANE_MODE_LANES) : ((lastLane + 2) % LANE_MODE_LANES));
      ...
      function buildHardLaneModeChart(songKey, laneCount = LANE_MODE_LANES, mode = "lanes") {
        const builtin = BUILTIN_MUSIC_TIMINGS[songKey];
        if (!builtin) return null;
        const groupWindowMs = songKey === "recordplayer" ? 108 : songKey === "palace" ? 118 : 165;
        const crowdedWindowMs = songKey === "recordplayer" ? 112 : songKey === "palace" ? 128 : 185;
        const grouped = [];
        ...
        const laneFreeUntil = new Array(laneCount).fill(-Infinity);
        const notes = [];
        let primaryLane = Math.max(0, Math.floor((laneCount - 1) / 2) - 1);
        let emitted = 0;

        function activeCountAt(timeMs) {
          return notes.filter((note) => note.timeMs <= timeMs && (note.endTimeMs || note.timeMs + 180) > timeMs).length;
        }
        ...
        function sideForLane(lane) {
          const midpoint = (laneCount - 1) * 0.5;
          if (lane < midpoint - 0.2) return "left";
          if (lane > midpoint + 0.2) return "right";
          return "center";
        }
        ...
        function hasSameHandLongOccupation(timeMs, side) {
          return activeLongNotesAt(timeMs).some((note) => {
            const span = getLaneNoteSpan(note);
            const lowSide = sideForLane(span.min);
            const highSide = sideForLane(span.max);
            if (lowSide === "center" || highSide === "center") return true;
            return lowSide === side || highSide === side;
          });
        }

        function occupiedHandSidesAt(timeMs) {
          const sides = new Set();
          activeLongNotesAt(timeMs).forEach((note) => {
            const span = getLaneNoteSpan(note);
            const lowSide = sideForLane(span.min);
            const highSide = sideForLane(span.max);
            if (lowSide === "center" || highSide === "center") {
              sides.add("left");
              sides.add("right");
              return;
            }
        ...
        function blockedByActiveLong(lane, timeMs) {
          return activeLongNotesAt(timeMs).some((note) => {
            ...
            const lockedSide = notes
              .filter((entry) => entry !== note && entry.timeMs <= timeMs && (entry.endTimeMs || entry.timeMs + 180) > timeMs)
              .map((entry) => {
                const otherSpan = getLaneNoteSpan(entry);
                if (otherSpan.max < span.min) return "left";
                if (otherSpan.min > span.max) return "right";
        ...
        function isTimeCrowded(timeMs) {
          return notes.some((note) => Math.abs(note.timeMs - timeMs) < crowdedWindowMs);
        }
        ...
        function pickLane(timeMs, preferred, avoid = [], requiredSide = "") {
          let available = availableLanesAt(timeMs).filter((lane) => !avoid.includes(lane) && !blockedByActiveLong(lane, timeMs));
          if (requiredSide) {
            const sideFiltered = available.filter((lane) => sideForLane(lane) === requiredSide);
            if (sideFiltered.length) available = sideFiltered;
          }
        ...
        function dragHandForSpan(startLane, endLane) {
          return sideForLane((startLane + endLane) * 0.5) === "left" ? "left" : "right";
        }
        ...
        function hasDragConflict(timeMs, endTimeMs, startLane, endLane) {
          return notes.some((note) => {
            ...
            const spanB = { min: Math.min(note.lane, note.endLane ?? note.lane), max: Math.max(note.lane, note.endLane ?? note.lane) };
            const overlap = spanA.min <= spanB.max + 1 && spanB.min <= spanA.max + 1;
            const reversedCross = (startLane - endLane) * ((note.lane ?? 0) - (note.endLane ?? note.lane ?? 0)) < 0;
            const sameHand = dragHandForSpan(startLane, endLane) === dragHandForSpan(note.lane, note.endLane ?? note.lane);
            return overlap || reversedCross || sameHand;
          });
        }

        function pushNote(timeMs, holdMs, lane, paired = false) {
          const shortHold = holdMs >= 220 && holdMs < 520;
          const longHold = holdMs >= 520;
          const wantsDrag = longHold && (emitted % 3 !== 1 || holdMs >= 900);
          const wantsFlick = !longHold && emitted % 9 === 4;
          const shortHoldAccent = shortHold && !paired && emitted % (laneCount >= 7 ? 10 : 12) === 3;
          const wantsHold = !wantsDrag && (longHold || shortHoldAccent);

          if (wantsDrag) {
            const targetPool = Array.from({ length: laneCount }, (_, index) => index).filter((candidate) => candidate !== lane);
            const endLane = targetPool
              .slice()
              .sort((a, b) => (Math.abs(Math.abs(a - lane) - (laneCount >= 7 ? 3 : 2)) - Math.abs(Math.abs(b - lane) - (laneCount >= 7 ? 3 : 2))) || (Math.abs(a - primaryLane) - Math.abs(b - primaryLane)))[0];
            const duration = Math.max(860, holdMs + (paired ? 180 : 320));
            const dragSide = dragHandForSpan(lane, endLane);
            if (hasSameHandLongOccupation(timeMs, dragSide) || hasDragConflict(timeMs, timeMs + duration, lane, endLane)) {
              const note = {
                id: `${mode}-hard-hold-${notes.length}`,
                type: "hold",
                timeMs,
                lane,
            ...
            const note = {
              id: `${mode}-hard-drag-${notes.length}`,
              type: "drag",
              timeMs,
              lane,
              endLane,
          ...
          const note = {
            id: `${mode}-hard-${wantsFlick ? "flick" : "tap"}-${notes.length}`,
        ...
        grouped.forEach((group, groupIndex) => {
          ...
          if (activeLongNotesAt(baseTime).length >= 2) return;
          if (isTimeCrowded(baseTime)) return;
          const availableCapacity = remainingCapacityAt(baseTime);
          if (availableCapacity < 1) return;
          const occupiedSides = occupiedHandSidesAt(baseTime);
          const requiredSide = occupiedSides.has("left") && !occupiedSides.has("right")
            ? "right"
            : occupiedSides.has("right") && !occupiedSides.has("left")
              ? "left"
              : "";
          ...
          const activeLongCount = activeLongNotesAt(baseTime).length;
          const palaceBoost = songKey === "palace";
          const desiredCount = (activeLongCount > 0 || longLead || secondLong)
            ? 1
            : ((sorted.length > 1 || groupIndex % (palaceBoost ? 2 : 4) === 0 || laneCount >= 7) ? 2 : 1);
          const targetCount = Math.min(desiredCount, availableCapacity);
          const preferredPattern = laneCount >= 7
            ? [2, 4, 1, 5, 3, 0, 6, 3, 1, 5]
            : [1, 2, 0, 3, 2, 1, 3, 0];
          const preferred = preferredPattern[groupIndex % preferredPattern.length];
          const firstLane = pickLane(baseTime, preferred, [], requiredSide);
          if (firstLane == null) return;
          pushNote(sorted[0][0], sorted[0][1], firstLane, targetCount > 1);

          if (targetCount < 2 || activeCountAt(baseTime) >= 2) return;
          const occupiedAfterFirst = occupiedHandSidesAt(baseTime);
          const secondRequiredSide = occupiedAfterFirst.has("left") && !occupiedAfterFirst.has("right")
            ? "right"
            : occupiedAfterFirst.has("right") && !occupiedAfterFirst.has("left")
              ? "left"
              : "";
          const opposite = firstLane < laneCount / 2 ? laneCount - 1 : 0;
          const secondLane = pickLane(baseTime, opposite, [firstLane], secondRequiredSide);
          if (secondLane == null) return;
          const secondEntry = sorted[1] || [baseTime, 0];
          if (sorted[0]?.type === "flick" && sorted[1]?.type === "flick") {
            // Allowed in 7-lane, but regular note typing already keeps this bounded.
          }
      ...
      function getBuiltinLaneChart(songKey, difficulty = "normal", mode = "lanes") {
        if (mode === "sevenlanes") {
          const cacheKey = `${songKey}:sevenlanes:hard`;
          if (!builtinHardLaneChartCache.has(cacheKey)) {
            const chart = buildHardLaneModeChart(songKey, SEVEN_LANE_MODE_LANES, "sevenlanes");
            if (chart) builtinHardLaneChartCache.set(cacheKey, chart);
          }
          return builtinHardLaneChartCache.get(cacheKey) || null;
        }
        if (difficulty === "hard") {
          const cacheKey = `${songKey}:lanes:hard`;
          if (!builtinHardLaneChartCache.has(cacheKey)) {
            const chart = buildHardLaneModeChart(songKey, LANE_MODE_LANES, "lanes");
            if (chart) builtinHardLaneChartCache.set(cacheKey, chart);
          }
          return builtinHardLaneChartCache.get(cacheKey) || null;
        }
      ...
      function getTrackTime(state) {
        if (state.preRollStartPerf) {
          return (state.preRollTrackOffsetMs || 0) + performance.now() - state.preRollStartPerf - state.preRollMs;
        }
        return state.audio ? state.audio.currentTime * 1000 : 0;
      }

      function getFreeformPathOrder(state, note = null, kind = "start") {
        const pathIndex = state.freeformPathIndex ?? 0;
        const path = FREEFORM_PATHS[pathIndex] || FREEFORM_PATHS[0];
        const cursor = state.freeformPathCursor ?? 0;
        const direction = state.freeformPathDirection ?? 1;
        const ahead = kind === "end" ? 2 : 0;
        const primary = [];
        for (let offset = 0; offset < path.length; offset += 1) {
          const rawIndex = cursor + ahead + offset * direction;
          const wrapped = ((rawIndex % path.length) + path.length) % path.length;
          primary.push({ x: path[wrapped][0], y: path[wrapped][1], _index: wrapped, _pathIndex: pathIndex });
        }
        const fallback = MUSIC_GAME_LANES.map(([x, y], idx) => ({ x, y, _index: idx, _pathIndex: -1 }));
        return primary.concat(fallback);
      }

      function advanceFreeformPath(state, note = null) {
        const path = FREEFORM_PATHS[state.freeformPathIndex ?? 0] || FREEFORM_PATHS[0];
        const step = note?.type === "drag" ? 2 : 1;
        const current = state.freeformPathCursor ?? 0;
        const direction = state.freeformPathDirection ?? 1;
        let next = current + step * direction;
        state.freeformPatternStepCount = (state.freeformPatternStepCount || 0) + 1;
        if (next >= path.length) {
          state.freeformPathSweepCount = (state.freeformPathSweepCount || 0) + 1;
          if ((state.freeformPathSweepCount || 0) >= 1) {
            state.freeformPathSweepCount = 0;
            state.freeformPathIndex = (((state.freeformPathIndex ?? 0) + 1) % FREEFORM_PATHS.length + FREEFORM_PATHS.length) % FREEFORM_PATHS.length;
            state.freeformPathDirection = (state.freeformPathIndex % 2 === 0) ? 1 : -1;
            const nextPath = FREEFORM_PATHS[state.freeformPathIndex] || FREEFORM_PATHS[0];
            next = state.freeformPathDirection > 0 ? 0 : nextPath.length - 1;
          } else {
            state.freeformPathDirection = -1;
            next = Math.max(0, path.length - 2);
          }
        } else if (next < 0) {
          state.freeformPathSweepCount = (state.freeformPathSweepCount || 0) + 1;
          if ((state.freeformPathSweepCount || 0) >= 1) {
            ...
            state.freeformPathDirection = 1;
            next = Math.min(path.length - 1, 1);
          }
        }
        state.freeformPathCursor = next;
      }

      function pickResponsivePoint(state, rect, exclude = [], note = null, kind = "start") {
        ...
        getFreeformPathOrder(state, note, kind).forEach((candidate, rank) => {
          const idx = candidate._index ?? rank;
          ...
          const pathBias = candidate._pathIndex >= 0 ? Math.max(0, 180 - rank * 26) : -90;
          const wave = Math.sin((Number(note?.timeMs || idx) * 0.011) + idx * 1.29) * 8;
          const score = minActive + centerBias + pathBias + wave - recentPenalty;
          if (score > bestScore) {
      ...
      function launchNumberBoom() {
        const palette = ["#ff4d38", "#ffe14d", "#ffffff"];
        applyPalette(palette);
        overlay.classList.add("effect-numberboom");

        const number = document.createElement("div");
        number.className = "secret-number-boom";
        number.textContent = "800000088";
        overlay.appendChild(number);

        for (let i = 0; i < 3; i += 1) {
          const ring = document.createElement("span");
          ring.className = "secret-number-shockwave";
          ring.style.setProperty("--delay", `${680 + i * 170}ms`);
          ring.style.setProperty("--size", `${minViewportRing(i)}px`);
          overlay.appendChild(ring);
        }

        const digits = "800000088";
        for (let i = 0; i < 72; i += 1) {
          const angle = (Math.PI * 2 * i) / 72 + (i % 7) * 0.08;
          const distance = 120 + (i % 6) * 38 + Math.random() * 90;
          createParticle(digits[i % digits.length], palette, {
            className: "phase-numberboom-digit",
            x: "50%",
            y: "50%",
            size: 24 + (i % 5) * 8,
            dx: `${Math.cos(angle) * distance}px`,
            dy: `${Math.sin(angle) * distance}px`,
            dur: `${1850 + Math.random() * 900}ms`,
            delay: `${760 + Math.random() * 380}ms`,
            rot: `${-240 + Math.random() * 480}deg`,
            core: `${palette[i % palette.length]}dd`,
            opacity: "1",
          });
        }
        ...
        for (let i = 0; i < 30; i += 1) {
          ...
          const distance = 80 + Math.random() * 210;
          createParticle(i % 3 === 0 ? "✦" : "•", palette, {
            className: "phase-numberboom-spark",
            x: "50%",
            y: "50%",
            size: 12 + Math.random() * 18,
            dx: `${Math.cos(angle) * distance}px`,
            dy: `${Math.sin(angle) * distance}px`,
            dur: `${1300 + Math.random() * 800}ms`,
            delay: `${500 + Math.random() * 520}ms`,
            rot: `${Math.random() * 360}deg`,
            core: `${palette[(i + 1) % palette.length]}ee`,
            opacity: "1",
          });
      ...
      function minViewportRing(index) {
        return 220 + index * 110;
      }

      function launchPiuliulIriu() {
        const palette = ["#00f5ff", "#ff3df2", "#fff44f"];
        applyPalette(palette);
        overlay.classList.add("effect-piuliul");

        const word = document.createElement("div");
        word.className = "secret-piuliul-word";
        word.textContent = "piuliul iriu";
        overlay.appendChild(word);

        for (let i = 0; i < 4; i += 1) {
          const vortex = document.createElement("span");
          vortex.className = "secret-piuliul-vortex";
          vortex.style.setProperty("--delay", `${220 + i * 130}ms`);
          vortex.style.setProperty("--size", `${180 + i * 92}px`);
          vortex.style.setProperty("--rot", `${i % 2 === 0 ? 1 : -1}`);
          overlay.appendChild(vortex);
        }

        const fragments = ["piu", "liul", "iriu", "piuliul", "iriu", "piuliul iriu"];
        for (let i = 0; i < 64; i += 1) {
          const angle = (Math.PI * 2 * i) / 64 + Math.sin(i) * 0.38;
          const distance = 90 + (i % 8) * 34 + Math.random() * 120;
          createParticle(fragments[i % fragments.length], palette, {
            className: "phase-piuliul-spin",
            x: "50%",
            y: "50%",
            size: 20 + (i % 6) * 6,
            dx: `${Math.cos(angle) * distance}px`,
            dy: `${Math.sin(angle) * distance}px`,
            dur: `${2100 + Math.random() * 1400}ms`,
            delay: `${520 + Math.random() * 760}ms`,
            rot: `${-720 + Math.random() * 1440}deg`,
            core: `${palette[i % palette.length]}cc`,
            opacity: "1",
          });
        }
        ...
        for (let i = 0; i < 36; i += 1) {
          ...
          const distance = 56 + (i % 5) * 36;
          createParticle(glyphs[i % glyphs.length], palette, {
            className: "phase-piuliul-chaos",
            x: `${50 + Math.cos(angle * 2.6) * 18}%`,
            y: `${50 + Math.sin(angle * 1.8) * 16}%`,
            size: 16 + (i % 4) * 7,
            dx: `${Math.cos(angle + Math.PI / 5) * distance}px`,
            dy: `${Math.sin(angle + Math.PI / 5) * distance}px`,
            dur: `${1600 + Math.random() * 1000}ms`,
            delay: `${1000 + i * 24}ms`,
            rot: `${i * 52}deg`,
            core: `${palette[(i + 1) % palette.length]}dd`,
            opacity: "1",
          });
        }
      }

      function launchSillyWord(type, word, options) {
        const palette = options.palette;
        applyPalette(palette);
        overlay.classList.add(`effect-${type}`);

        const title = document.createElement("div");
        title.className = `secret-silly-word ${options.wordClass}`;
        title.textContent = word;
        overlay.appendChild(title);

        for (let i = 0; i < options.ringCount; i += 1) {
          const pop = document.createElement("span");
          pop.className = `secret-silly-pop ${options.popClass}`;
          pop.style.setProperty("--delay", `${options.ringDelay + i * 120}ms`);
          pop.style.setProperty("--size", `${120 + i * 74}px`);
          pop.style.setProperty("--x", `${36 + Math.random() * 28}%`);
          pop.style.setProperty("--y", `${36 + Math.random() * 28}%`);
          overlay.appendChild(pop);
        }

        for (let i = 0; i < options.count; i += 1) {
          const angle = (Math.PI * 2 * i) / options.count + (options.angleJitter ? Math.random() * options.angleJitter : 0);
          const distance = options.distance + (i % 7) * options.distanceStep + Math.random() * options.randomDistance;
          const text = options.bits[i % options.bits.length];
          createParticle(text, palette, {
            className: options.particleClass,
            x: options.originX ? `${options.originX(i)}%` : "50%",
            y: options.originY ? `${options.originY(i)}%` : "50%",
            size: options.size + (i % 5) * options.sizeStep,
            dx: `${Math.cos(angle) * distance}px`,
            dy: `${Math.sin(angle) * distance}px`,
            dur: `${options.duration + Math.random() * options.durationJitter}ms`,
            delay: `${options.delay + Math.random() * options.delayJitter}ms`,
            rot: `${options.rotBase + Math.random() * options.rotJitter}deg`,
            core: `${palette[i % palette.length]}cc`,
            opacity: "1",
          });
        }
      ...
      function launchBiupi() {
        launchSillyWord("biupi", "biupi", {
          palette: ["#ff4fd8", "#fff24d", "#57f7ff"],
          wordClass: "hop",
          popClass: "bubble",
          particleClass: "phase-silly-hop",
          bits: ["biu", "pi", "biupi", "pop"],
          count: 34,
          distance: 70,
          distanceStep: 18,
          randomDistance: 80,
      ...
      function launchLapi() {
        launchSillyWord("lapi", "lapi", {
          palette: ["#7cff4f", "#ff9f43", "#ffffff"],
          wordClass: "squash",
          popClass: "square",
          particleClass: "phase-silly-flop",
          bits: ["la", "pi", "lapi", "plap"],
          count: 38,
          distance: 60,
          distanceStep: 14,
          randomDistance: 120,
          ...
          angleJitter: 1.1,
          originX: (i) => 18 + (i % 9) * 8,
          originY: (i) => 42 + Math.sin(i) * 18,
        });
      }

      function launchBiupiBiupi() {
        launchSillyWord("biupibiupi", "biupibiupi", {
          palette: ["#35a7ff", "#ff4f79", "#f9ff60"],
          wordClass: "spin",
          popClass: "burst",
          particleClass: "phase-silly-tornado",
          bits: ["biupi", "biupi", "BIU", "PI"],
          count: 58,
          distance: 90,
          distanceStep: 24,
          randomDistance: 120,
      ...
      function launchLapiLapi() {
        launchSillyWord("lapilapi", "lapilapi", {
          palette: ["#b45cff", "#5cff9a", "#ffea5c"],
          wordClass: "zigzag",
          popClass: "diamond",
          particleClass: "phase-silly-zigzag",
          bits: ["lapi", "lapi", "LA", "PI", "wob"],
          count: 54,
          distance: 84,
          distanceStep: 21,
          randomDistance: 130,
          ...
          ringDelay: 120,
          angleJitter: 0.9,
          originX: (i) => 50 + Math.cos(i * 1.7) * 16,
          originY: (i) => 50 + Math.sin(i * 2.1) * 16,
        });
      }
      ...
      async function loadSongChart(songKey, mode = "freeform", difficulty = "normal") {
        ...
        const builtin = (mode === "lanes" || mode === "sevenlanes")
          ? getBuiltinLaneChart(songKey, difficulty, mode)
      ...
      function prepareSongData(songKey, mode, difficulty, loaded) {
        ...
        const rawChart = loaded.chart.map((note) => ({ ...note }));
        const chart = sanitizePlayableChart(rawChart, mode);
        const prepared = {
          ...loaded,
          chart,
      ...
      function sanitizePlayableChart(chart, mode = "freeform") {
        if (!Array.isArray(chart) || !chart.length) return [];
        const sorted = chart.slice().sort((a, b) => {
          const timeDelta = (a.timeMs - b.timeMs);
          if (timeDelta) return timeDelta;
          const aLong = a.endTimeMs ? 1 : 0;
          const bLong = b.endTimeMs ? 1 : 0;
          if (aLong !== bLong) return bLong - aLong;
          return (a.lane ?? 0) - (b.lane ?? 0);
        });
        const kept = [];
        const overlapTailMs = mode === "freeform" ? 220 : 180;

        function noteEndTime(note) {
          return note.endTimeMs || (note.timeMs + overlapTailMs);
        }

        function noteStartsBefore(note, timeMs) {
          return (note.timeMs ?? 0) <= timeMs;
        }

        function noteActiveAt(note, timeMs) {
          return noteStartsBefore(note, timeMs) && timeMs < noteEndTime(note);
        }

        function activeAt(timeMs) {
          return kept.filter((entry) => noteActiveAt(entry, timeMs));
        }

        function activeLongAt(timeMs) {
          return activeAt(timeMs).filter((entry) => entry.endTimeMs);
        }

        function relativeSideOf(note, anchor) {
          const noteSpan = getLaneNoteSpan(note);
          const anchorSpan = getLaneNoteSpan(anchor);
          if (noteSpan.max < anchorSpan.min) return "left";
          if (noteSpan.min > anchorSpan.max) return "right";
          return "";
        }

        function violatesDragHandBlock(note) {
          if (mode !== "lanes" && mode !== "sevenlanes") return false;
          const samples = [note.timeMs];
          if (note.endTimeMs) samples.push(Math.max(note.timeMs, note.endTimeMs - 1));

          return samples.some((sampleTime) => {
            const activeDrags = activeLongAt(sampleTime).filter((entry) => entry.type === "drag");
            const span = getLaneNoteSpan(note);
            return activeDrags.some((drag) => {
              const dragHand = getLaneDragHand(drag, kept);
              const dragSpan = getLaneNoteSpan(drag);
              if (dragHand === "left" && span.max < dragSpan.min) return true;
              if (dragHand === "right" && span.min > dragSpan.max) return true;
              return false;
            });
          });
        }

        function violatesLongSideLock(note) {
          if (mode !== "lanes" && mode !== "sevenlanes") return false;
          const samples = [note.timeMs];
          if (note.endTimeMs) samples.push(Math.max(note.timeMs, note.endTimeMs - 1));

          return samples.some((sampleTime) => {
            const activeLongs = activeLongAt(sampleTime);
            return activeLongs.some((longNote) => {
              const noteSide = relativeSideOf(note, longNote);
              if (!noteSide) return false;
              const lockedSide = activeAt(sampleTime)
                .filter((entry) => entry !== longNote)
                .map((entry) => relativeSideOf(entry, longNote))
                .find((side) => side === "left" || side === "right");
              return Boolean(lockedSide && lockedSide !== noteSide);
            });
          });
        ...
        function exceedsFingerDemand(note) {
          if (mode !== "lanes" && mode !== "sevenlanes") return false;
          const samples = [note.timeMs];
          if (note.endTimeMs) {
            samples.push(Math.min(note.endTimeMs - 1, note.timeMs + 240));
            samples.push(Math.max(note.timeMs, note.endTimeMs - 1));
          }
          return samples.some((sampleTime) => {
            const active = activeAt(sampleTime);
            const activeLong = active.filter((entry) => entry.endTimeMs);
            if (active.length >= 2) return true;
            if (activeLong.length >= 2) return true;
            if (activeLong.length >= 1 && active.some((entry) => !entry.endTimeMs)) return true;
            return false;
          });
        }

        sorted.forEach((note) => {
          if (exceedsFingerDemand(note)) return;
          if (violatesDragHandBlock(note)) return;
          if (violatesLongSideLock(note)) return;
          kept.push(note);
        });

        if (mode === "lanes" || mode === "sevenlanes") {
          const sanitized = [];
          kept.forEach((note) => {
            const overlapTail = note.endTimeMs || (note.timeMs + overlapTailMs);
            const simultaneous = sanitized.filter((entry) => entry.timeMs === note.timeMs);
            const simultaneousLong = simultaneous.filter((entry) => entry.endTimeMs);
            if (!note.endTimeMs && simultaneousLong.length >= 2) return;
            const activeLong = sanitized.filter((entry) => entry.endTimeMs && entry.timeMs <= note.timeMs && note.timeMs < (entry.endTimeMs || entry.timeMs));
            if (!note.endTimeMs && activeLong.length >= 2) return;
            if (note.endTimeMs && activeLong.length >= 2) return;
            const activeAny = sanitized.filter((entry) => entry.timeMs <= note.timeMs && note.timeMs < (entry.endTimeMs || (entry.timeMs + overlapTailMs)));
            if (activeLong.length >= 1 && activeAny.some((entry) => !entry.endTimeMs)) return;
            if (!note.endTimeMs && activeLong.length >= 1 && activeAny.length >= 1) return;
            sanitized.push({ ...note, endTimeMs: note.endTimeMs ? overlapTail : undefined, __actualEndTimeMs: overlapTail });
          });
          return sanitized.map((note, index) => ({
            ...note,
            endTimeMs: note.endTimeMs,
            id: note.id || `${mode}-${note.type}-${index}`,
          }));
        }

        return kept.map((note, index) => ({
          ...note,
          id: note.id || `${mode}-${note.type}-${index}`,
        }));
      }
      ...
      async function preloadMusicGameData(state) {
        ...
        const tasks = [
          ...
          loadSongChart("fiu", "sevenlanes", "hard").then((loaded) => prepareSongData("fiu", "sevenlanes", "hard", loaded)),
          ...
          loadSongChart("palace", "sevenlanes", "hard").then((loaded) => prepareSongData("palace", "sevenlanes", "hard", loaded)),
          ...
          loadSongChart("lafantasia", "sevenlanes", "hard").then((loaded) => prepareSongData("lafantasia", "sevenlanes", "hard", loaded)),
          ...
          loadSongChart("recordplayer", "sevenlanes", "hard").then((loaded) => prepareSongData("recordplayer", "sevenlanes", "hard", loaded)),
          ...
          loadSongChart("stallion", "sevenlanes", "hard").then((loaded) => prepareSongData("stallion", "sevenlanes", "hard", loaded)),
      ...
      async function resumeMusicGame(state) {
        ...
        state.preRollMs = Math.max(state.approachMs + 120, 1800);
        state.preRollTrackOffsetMs = state.audio.currentTime * 1000;
        state.preRollStartPerf = performance.now();
        state.statusLabel.textContent = IS_ZH ? "继续准备…" : "Resume in...";
        state.running = true;
        if (state.rafId) cancelAnimationFrame(state.rafId);
        state.rafId = requestAnimationFrame(() => updateMusicGame(state));
        await showMusicGameCountdown(state);
        if (!musicGameState || musicGameState !== state || state.paused) return;
        state.running = false;
        if (state.rafId) cancelAnimationFrame(state.rafId);
        state.rafId = 0;
        state.preRollStartPerf = 0;
        await state.audio.play();
        state.songStartPerf = performance.now() - state.audio.currentTime * 1000;
      ...
      function renderResultsPanel(state) {
        ...
        const accuracy = Math.min(100, (state.score / maxScore) * 100).toFixed(2);
        panel.innerHTML = `
          ...
          <div class="music-game-results-actions">
            <button type="button" class="music-game-button music-game-results-replay">${IS_ZH ? "再来一次" : "Play again"}</button>
            <button type="button" class="music-game-button music-game-results-newgame">${IS_ZH ? "新游戏" : "New game"}</button>
          </div>
        `;
      ...
      function addScore(state, amount, x, y, label, judgment = "great", note = null) {
        ...
        if (state.autoPlay) {
          ...
          if (note) finalAmount = getPerfectScoreForNote(note, state.mode);
        }
        state.score += finalAmount;
        state.combo += 1;
        state.maxCombo = Math.max(state.maxCombo, state.combo);
        if (finalJudgment === "perfect") state.perfectCount += 1;
      ...
      function getLaneDragHand(note, chart = []) {
        ...
        const span = getLaneNoteSpan(note);
        const laneMidpoint = span.max > 3 ? 3 : 1.5;
        const defaultHand = averageLane < laneMidpoint ? "left" : "right";
      ...
      function runAutoPlay(state, trackTime) {
        if (!state.autoPlay) return;
        if (state.mode === "lanes" || state.mode === "sevenlanes") {
          state.chart.forEach((note) => {
            if (!note.element || note.hit || note.missed) return;
            if (!note.started && trackTime >= note.timeMs - 8) {
              if (note.type === "tap") {
                completeLaneNote(state, note, getPerfectScoreForNote(note, state.mode), resolveScoreLabel(note, "Perfect"), "perfect");
                return;
              }
              if (note.type === "flick") {
                completeLaneNote(state, note, getPerfectScoreForNote(note, state.mode), resolveScoreLabel(note, IS_ZH ? "轻扫完成" : "Flick"), "perfect");
                return;
              }
              note.started = true;
      ...
      function createLaneNoteElement(state, note) {
        ...
        root.className = `music-game-lane-note type-${note.type}${note.bonus ? " is-bonus" : ""}${dragHand ? ` hand-${dragHand}` : ""}${state.mode === "sevenlanes" ? " direct-hit" : ""}`;
        root.innerHTML = `
      ...
      function findSevenLaneStartCandidate(state, target, trackTime) {
        const hitRoot = target?.closest?.(".music-game-lane-note");
        const directMatch = hitRoot
          ? state.chart
              .filter((note) => note.element === hitRoot && !note.hit && !note.missed && !note.started)
              .filter((note) => Math.abs(trackTime - note.timeMs) <= state.hitWindowMs + 40)
              .sort((a, b) => Math.abs(trackTime - a.timeMs) - Math.abs(trackTime - b.timeMs))[0] || null
          : null;
        if (directMatch) return directMatch;

        const probeX = state.pendingPointerX;
        const probeY = state.pendingPointerY;
        if (typeof probeX !== "number" || typeof probeY !== "number") return null;

        return state.chart
          .filter((note) => note.element && !note.hit && !note.missed && !note.started)
          .filter((note) => Math.abs(trackTime - note.timeMs) <= state.hitWindowMs + 55)
          .map((note) => {
            const rect = note.element.getBoundingClientRect();
            const margin = 22;
            const inside = probeX >= rect.left - margin
              && probeX <= rect.right + margin
              && probeY >= rect.top - margin
              && probeY <= rect.bottom + margin;
            if (!inside) return null;
            const cx = rect.left + rect.width * 0.5;
            const cy = rect.top + rect.height * 0.5;
            return { note, distance: Math.hypot(probeX - cx, probeY - cy) };
          })
          .filter(Boolean)
          .sort((a, b) => a.distance - b.distance || Math.abs(trackTime - a.note.timeMs) - Math.abs(trackTime - b.note.timeMs))[0]?.note || null;
      }
      ...
      function handleLanePointerDown(state, event) {
        if (!state.running || (state.mode !== "lanes" && state.mode !== "sevenlanes")) return;
        if (state.autoPlay) return;
        const trackTime = getTrackTime(state);
        event.preventDefault();
        state.pendingPointerX = event.clientX;
        state.pendingPointerY = event.clientY;
        const directNote = state.mode === "sevenlanes" ? findSevenLaneStartCandidate(state, event.target, trackTime) : null;
        const lane = directNote ? directNote.lane : getLaneInputZone(state, event.clientX, event.clientY, true);
        if (lane == null && !directNote) return;
        const note = directNote || findLaneStartCandidate(state, lane, trackTime);
        if (!note) return;
        state.playfield.setPointerCapture?.(event.pointerId);
        tryStartLaneNote(state, note, event.pointerId, note.lane, event.clientX, event.clientY, trackTime);
      }
      ...
      function handleLanePointerEnd(state, event, canceled = false) {
        if (!state.mode || (state.mode !== "lanes" && state.mode !== "sevenlanes")) return;
        if (state.autoPlay) return;
        const pointer = state.activeLanePointers.get(event.pointerId);
        if (!pointer) return;
        const { note } = pointer;
      ...
      function spawnNoteElement(state, note) {
        ...
        if (note.type === "drag") {
          const endPoint = pickResponsivePoint(state, rect, [startPoint], note, "end");
          note.endX = endPoint.x;
          note.endY = endPoint.y;
        ...
        advanceFreeformPath(state, note);
      ...
      async function startMusicSong(songKey, mode = "freeform", difficulty = "normal") {
        ...
        state.approachMs = (mode === "lanes" || mode === "sevenlanes") ? 920 : 1050;
        state.hitWindowMs = mode === "sevenlanes" ? 185 : ((mode === "lanes") ? 150 : 185);
        state.laneCount = mode === "sevenlanes" ? SEVEN_LANE_MODE_LANES : (mode === "lanes" ? LANE_MODE_LANES : 0);
        state.activeLanePointers = new Map();
        state.laneColumns = [];
        state.freeformRecentPointIndexes = [];
        state.preRollTrackOffsetMs = 0;
        const freeformSeed = `${songKey}:${difficulty}`;
        const freeformPathIndex = Array.from(freeformSeed).reduce((sum, char) => sum + char.charCodeAt(0), 0) % FREEFORM_PATHS.length;
        state.freeformPathIndex = freeformPathIndex;
        state.freeformPathDirection = freeformPathIndex % 2 === 0 ? 1 : -1;
        state.freeformPathCursor = state.freeformPathDirection > 0 ? 0 : (FREEFORM_PATHS[freeformPathIndex].length - 1);
        state.freeformPatternStepCount = 0;
        state.paused = false;
        state.pauseOverlay?.classList.remove("active");
        musicGameLayer.classList.remove("is-paused");
        removeResultsPanel(state);
        ...
        if (state.modeLabel) {
          state.modeLabel.textContent = mode === "lanes"
            ? (IS_ZH ? "下落模式" : "Lane mode")
            : mode === "sevenlanes"
              ? (IS_ZH ? "七轨模式" : "7 lane")
              : (IS_ZH ? "自由模式" : "Freeform");
        }
        ...
        state.playfield.classList.toggle("lane-mode", mode === "lanes" || mode === "sevenlanes");
        if (mode === "lanes" || mode === "sevenlanes") buildLaneModeScaffold(state);

        try {
          await preloadSongAudio(songKey).catch(() => song.audioSrc);
          state.audio = clonePreloadedAudio(songKey);
          if (!state.audio.src) state.audio.src = song.audioSrc;
          state.audio.load?.();
          const preparedKey = `${songKey}:${mode}:${difficulty}`;
      ...
      function buildMusicGameUi() {
          ...
          <div class="music-game-setup">
            ...
            <div class="music-game-step" data-step="mode">
              <div class="music-game-mode-list">
                ...
                <button type="button" class="music-game-button music-game-mode-button" data-mode="sevenlanes">${IS_ZH ? "七轨模式" : "7 lane"}</button>
              </div>
              <div class="music-game-step-actions">
                <button type="button" class="music-game-button music-game-back-button" data-back-step="download">${IS_ZH ? "上一步" : "Back"}</button>
              </div>
            </div>
            <div class="music-game-step" data-step="song">
              ...
              <div class="music-game-step-actions">
                <button type="button" class="music-game-button music-game-back-button" data-back-step="mode">${IS_ZH ? "上一步" : "Back"}</button>
              </div>
            </div>
            <div class="music-game-step" data-step="options">
              ...
              <div class="music-game-step-actions">
                <button type="button" class="music-game-button music-game-back-button" data-back-step="song">${IS_ZH ? "上一步" : "Back"}</button>
              </div>
            </div>
          </div>
          ...
          <div class="music-game-pause-overlay">
            <div class="music-game-pause-card">
              ...
              <div class="music-game-pause-actions">
                ...
                <button type="button" class="music-game-button music-game-replay">${IS_ZH ? "再来一次" : "Play again"}</button>
        ...
        const replayButton = shell.querySelector(".music-game-replay");
        const quitButton = shell.querySelector(".music-game-quit");
        const backButtons = shell.querySelectorAll(".music-game-back-button");
        ...
        function enterFreshMusicSetup() {
          if (!musicGameState) return;
          musicGameState.running = false;
          musicGameState.paused = false;
          musicGameState.selectedMode = "";
          musicGameState.selectedSong = "";
          musicGameState.selectedLevel = "normal";
          musicGameState.setupStepOverride = "";
          musicGameState.chart = [];
          musicGameState.nextSpawnIndex = 0;
          musicGameState.score = 0;
          musicGameState.combo = 0;
          musicGameState.maxCombo = 0;
          musicGameState.missCount = 0;
          musicGameState.perfectCount = 0;
          musicGameState.greatCount = 0;
          musicGameState.maxPossibleScore = 0;
          if (musicGameState.rafId) cancelAnimationFrame(musicGameState.rafId);
          musicGameState.rafId = 0;
          musicGameState.timeouts.forEach((timeoutId) => clearTimeout(timeoutId));
          musicGameState.timeouts = [];
          musicGameState.audio?.pause();
          if (musicGameState.audio) musicGameState.audio.currentTime = 0;
          musicGameState.playfield.replaceChildren();
          musicGameState.pauseOverlay?.classList.remove("active");
          removeResultsPanel(musicGameState);
          musicGameLayer.classList.remove("is-playing", "is-paused");
          musicGameState.scoreValue.textContent = "0";
          musicGameState.comboValue.textContent = "0";
          musicGameState.songLabel.textContent = "-";
          musicGameState.statusLabel.textContent = IS_ZH ? "请选择模式" : "Choose a mode.";
          syncModeUi();
          syncLevelUi();
          syncSongUi();
          syncSetupFlow();
        ...
        function getSetupStep() {
          ...
          if (musicGameState.setupStepOverride) return musicGameState.setupStepOverride;
        ...
        function syncLevelUi() {
          ...
          const sevenLaneOnly = musicGameState.selectedMode === "sevenlanes";
          if (sevenLaneOnly) musicGameState.selectedLevel = "hard";
          const selectedLevel = sevenLaneOnly ? "hard" : (musicGameState.selectedLevel || "normal");
          levelList.querySelectorAll("[data-level]").forEach((button) => {
            const isHardButton = button.getAttribute("data-level") === "hard";
            button.classList.toggle("is-active", button.getAttribute("data-level") === selectedLevel);
            button.disabled = sevenLaneOnly && !isHardButton;
            button.hidden = sevenLaneOnly && !isHardButton;
          });
        ...
        downloadButton.addEventListener("click", () => {
          preloadMusicGameData(musicGameState).then(() => {
            if (musicGameState) musicGameState.setupStepOverride = "";
            syncSetupFlow();
          }).catch(() => {});
        });
        ...
        modeList.addEventListener("click", (event) => {
          ...
          musicGameState.selectedSong = "";
          musicGameState.selectedLevel = musicGameState.selectedMode === "sevenlanes" ? "hard" : "normal";
        ...
        quitButton.addEventListener("click", () => {
          enterFreshMusicSetup();
        });

        backButtons.forEach((button) => {
          button.addEventListener("click", () => {
            if (!musicGameState) return;
            const backStep = button.getAttribute("data-back-step") || "";
            if (backStep === "song") {
              musicGameState.selectedSong = "";
            } else if (backStep === "mode") {
              musicGameState.selectedMode = "";
              musicGameState.selectedSong = "";
            }
            musicGameState.setupStepOverride = backStep;
            syncModeUi();
            syncLevelUi();
            syncSongUi();
            syncSetupFlow();
        ...
        playfield.addEventListener("click", (event) => {
          const replayTarget = event.target.closest(".music-game-results-replay");
          if (replayTarget && musicGameState?.selectedSong && musicGameState.selectedMode) {
            startMusicSong(
              musicGameState.selectedSong,
              musicGameState.selectedMode,
              musicGameState.selectedLevel || "normal"
          ...
          if (event.target.closest(".music-game-results-newgame")) {
            enterFreshMusicSetup();
          }
        });
      ...
      function launchEffect(type) {
        ...
        else if (type === "numberboom") launchNumberBoom();
        else if (type === "piuliul") launchPiuliulIriu();
        else if (type === "biupi") launchBiupi();
        else if (type === "lapi") launchLapi();
        else if (type === "biupibiupi") launchBiupiBiupi();
        else if (type === "lapilapi") launchLapiLapi();
        ...
        cleanupTimer = setTimeout(clearEffect, type === "mama" ? 6200 : type === "piuliul" ? 5600 : type === "biupibiupi" || type === "lapilapi" ? 5200 : type === "biupi" || type === "lapi" ? 4200 : type === "numberboom" ? 4400 : type === "polarbear" ? 6200 : type === "hearts" ? 4200 : type === "fiush" ? 7200 : type === "spiretz" ? 6200 : type === "fiu" ? 4200 : type === "midnight" ? 7600 : type === "iceworld" ? 9200 : type === "elements" ? 6200 : type === "thunder" ? 7800 : type === "laser" ? 7200 : type === "violet" ? 7600 : type === "halo" ? 3800 : 3200);
      }
