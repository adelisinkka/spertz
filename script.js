    ...
    (function () {
      ...
      window.SPERTZ_ZH_MEANING = ZH_MEANING;
      ...
      const QUIZ_ZH = {
        ...
        "-or (suffix)": "-or（后缀）",
        "mi (particle)": "mi（助词）",
        "zequk / ze at the beginning": "句首加 zequk / ze",
      ...
      const QUIZ_PROMPT_ZH = {
        "In Spertz, <c> before <i> is pronounced:": "在 Spertz 中，<c> 位于 <i> 前时读作：",
        ...
        "Progressive aspect marker is:": "进行体标记是：",
        ...
        "Declarative/Indicative mood suffix is:": "陈述语气后缀是：",
      ...
      function zhQuizText(text) {
        ...
        if ((m = t.match(/^Digit word for (\d+) \(base-10\) is:$/))) return `十进制数字 ${m[1]} 的 Spertz 数词是：`;
        if ((m = t.match(/^Type the Spertz digit word for (\d+)\.$/))) return `请输入数字 ${m[1]} 的 Spertz 数词。`;
        if ((m = t.match(/^Which conjunction means "(.+)"\?$/))) return `哪个连词表示“${zhTerm(m[1])}”？`;
        if ((m = t.match(/^Which month is "(.+)"\?$/))) return `“${m[1]}”是哪个月份？`;
        if ((m = t.match(/^What is the Spertz name for (.+)\?$/))) return `${zhTerm(m[1])} 的 Spertz 名称是什么？`;
        if ((m = t.match(/^Is "(.+)" an allowed syllable pattern in words \(including names\)\?$/))) return `“${m[1]}”是允许的音节结构吗（包括姓名）？`;
        if ((m = t.match(/^Choose the correct Spertz verb form for: "(.+)"$/))) return `请选择正确的 Spertz 动词形式：\n“${m[1]}”`;
      ...
      function glossFromConfig(config) {
        ...
        const subjEn = IS_ZH
          ? (
            ...
            config.subj === "2sg" ? "你" :
            config.subj === "2pl" ? "你们" :
            config.subj === "3sg" ? "他/她/TA" :
            config.subj === "3pl" ? "他们/她们/TA们" :
            "（某人）"
          )
        ...
        let aspectNote = "";
        if (config.aspect === "ok") aspectNote = IS_ZH ? "（进行中）" : " (ongoing)";
    ...
    (function () {
      ...
      function buildCardCanvas(data) {
        ...
        ctx.fillText(IS_ZH ? "SPERTZ 人格卡片" : "SPERTZ Personality Card", 48, 70);
    ...
    (function () {
      ...
      function buildCardCanvas(result, userTraits) {
        ...
        ctx.fillText(IS_ZH ? "SPERTZ 动物匹配" : "SPERTZ Animal Match", 48, 70);
      ...
      function calculate() {
        ...
        resultsBox.innerHTML = `
          <div class="personality-card">
            <h3 style="margin:0;">${IS_ZH ? "你的动物匹配" : "Your animal match"}: ${best.name}</h3>
    ...
    (function () {
      ...
      function buildCardCanvas(rings) {
        ...
        ctx.fillStyle = heading;
        ctx.font = "700 26px serif";
        ctx.fillText(IS_ZH ? "超能力日戒环" : "Superpower Day Rings", 48, 70);

        ctx.fillStyle = text;
        ctx.font = "16px sans-serif";
        wrapText(ctx, IS_ZH ? "Pesimura · 12月26日 · 成年时觉醒戒环。" : "Pesimura · December 26 · Rings awaken when you reach adulthood.", 48, 98, 800, 22);

        ctx.fillStyle = accent;
        ctx.font = "700 18px serif";
        ctx.fillText(IS_ZH ? `戒环数：${rings.length}` : `Rings: ${rings.length}`, 48, 140);
        ...
        rings.forEach((ring, idx) => {
          ...
          ctx.fillStyle = accent2;
          ctx.font = "700 18px serif";
          ctx.fillText(IS_ZH ? `戒环 ${idx + 1}` : `Ring ${idx + 1}`, innerX, y + 36);
          ctx.fillStyle = text;
          ctx.font = "16px sans-serif";
      ...
      function calculate() {
        ...
        resultsBox.innerHTML = `
          <div class="personality-card">
            <h3 style="margin:0;">${IS_ZH ? "你的戒环" : "Your Rings"}</h3>
            <p class="mini-note" style="margin-top:8px;">
              ${IS_ZH ? "在 Pesimura，成年当年的 12 月 26 日会觉醒戒环。" : "Rings awaken in Pesimura on December 26, the year you become an adult."}
            </p>
            <div class="result-pills" style="margin-top:10px;">
              <span class="result-pill">${IS_ZH ? "戒环数" : "Rings"}: ${ringsView.length}</span>
            </div>
          </div>
          <div class="personality-card">
            ${ringsView.map((r, idx) => `
              <div class="rule-note" style="margin-top:10px; border-color: rgba(255,255,255,0.18); background:${ringGradient(r.properties || [r.property])}; color:#f5efe6; box-shadow:0 12px 26px rgba(0,0,0,0.18);">
                <p style="margin:0; color:#f5efe6;"><b>${IS_ZH ? "戒环" : "Ring"} ${idx + 1}</b> · ${IS_ZH ? "属性" : "Property"}: ${r.property}</p>
    ...
    (function () {
      ...
      function buildCarCard(best, scores, selections, overallScore) {
        ...
        ctx.fillStyle = heading;
        ctx.font = "700 26px serif";
        ctx.fillText(IS_ZH ? "SPERTZ 车辆创新" : "SPERTZ Car Innovation", 48, 70);
...
refreshAll(true);
  ...
  (function () {
    ...
    const VERB_ZH_MEANING = window.SPERTZ_ZH_MEANING || {};
    ...
    function roughGloss(formInfo) {
      ...
      let aspectNote = "";
      if (aspect === "ok") aspectNote = IS_ZH ? "（进行中）" : " (ongoing)";
...
(function () {
  ...
  randomBirthBtn.addEventListener("click", randomBirthday);
    ...
    (function () {
      ...
      if (!tbody) return;
      const meaningMap = window.SPERTZ_ZH_MEANING || {};
