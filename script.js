    ...
    (function () {
      ...
      function materializeBuiltinMusicChart(songKey) {
        const builtin = BUILTIN_MUSIC_TIMINGS[songKey];
        if (!builtin) return null;
        const compressedMelody = compressMelodyTimeline(builtin.melody);
        return {
          bpm: builtin.bpm,
      ...
      function pickResponsivePoint(state, rect, exclude = []) {
        ...
        MUSIC_GAME_LANES.forEach(([x, y], idx) => {
          ...
          const score = minActive + centerBias + idx * 0.01;
          if (score > bestScore) {
            best = candidate;
            bestScore = score;
          }
      ...
      async function loadSongChart(songKey, mode = "freeform") {
        const cacheKey = `${songKey}:${mode}`;
        ...
        const builtin = mode === "lanes"
          ? BUILTIN_LANE_MODE_CHARTS[songKey]
          : (materializeBuiltinMusicChart(songKey) || BUILTIN_MUSIC_CHARTS[songKey]);
        if (builtin) {
      ...
      function preloadSongAudio(songKey) {
        if (musicAudioCache.has(songKey)) return musicAudioCache.get(songKey);
        const song = MUSIC_SONGS[songKey];
        ...
        musicAudioCache.set(songKey, promise);
        return promise;
      }
      ...
      async function preloadMusicGameData(state) {
        ...
        const tasks = [
          loadSongChart("fiu", "freeform"),
          loadSongChart("fiu", "lanes"),
          preloadSongAudio("fiu"),
        ];
        state.downloadPromise = Promise.all(tasks)
          .then(() => {
            state.assetsDownloaded = true;
            state.statusLabel.textContent = IS_ZH ? "数据已下载" : "Data downloaded";
          })
          .catch((error) => {
            state.assetsDownloaded = false;
            const message = error && typeof error.message === "string" ? error.message : String(error || "Unknown error");
            state.statusLabel.textContent = IS_ZH ? `下载失败：${message}` : `Download failed: ${message}`;
            throw error;
          })
          .finally(() => {
      ...
      function teardownMusicGame() {
        ...
        musicGameLayer.classList.remove("active", "is-playing", "is-song-list-open");
        musicGameState = null;
      }
      ...
      function addScore(state, amount, x, y, label) {
        state.score += amount;
        state.combo += 1;
      ...
      function completeLaneNote(state, note, amount, label) {
        if (note.hit || note.missed) return;
        note.hit = true;
        note.completed = true;
        note.element?.classList.add("is-hit");
        const point = note.type === "drag" ? getLaneScreenPoint(state, note.endLane ?? note.lane) : getLaneScreenPoint(state, note.lane);
        addScore(state, amount, point.x, point.y, label);
      ...
      function createLaneNoteElement(state, note) {
        ...
        root.className = `music-game-lane-note type-${note.type}`;
        root.innerHTML = `
      ...
      function updateLaneNoteVisual(state, note, trackTime) {
        ...
        const endX = getLaneCenterX(state, endLane, layout);
        ...
        const headX = note.type === "drag" && note.started
          ? startX + (endX - startX) * holdProgress
          : startX;
        const tailX = note.type === "drag" ? endX : startX;
      ...
      function tryStartLaneNote(state, note, pointerId, lane, clientX, clientY, trackTime) {
        ...
        if (note.type === "tap") {
          completeLaneNote(state, note, delta <= 70 ? 120 : 90, delta <= 70 ? "Perfect" : "Great");
          return true;
        }
      ...
      function handleLanePointerMove(state, event) {
        ...
        if (note.type === "flick") {
          ...
          if (travel >= 34 && trackTime <= note.timeMs + state.hitWindowMs + 180) {
            completeLaneNote(state, note, 140, IS_ZH ? "轻扫完成" : "Flick");
          }
        }
      }
      ...
      function createTapNoteElement(state, note, rect) {
        ...
        el.type = "button";
        el.className = "music-game-note music-game-note-tap";
        ...
        el.addEventListener("pointerdown", (event) => {
          ...
          addScore(state, delta < 70 ? 120 : 80, note.x * rect.width, note.y * rect.height, delta < 70 ? "Perfect" : "Great");
      ...
      function maybeCompleteActiveDrag(state, event) {
        ...
        if (timeDelta <= state.hitWindowMs + 60 && distance <= 46) {
          ...
          addScore(state, timeDelta < 80 ? 180 : 120, targetX, targetY, timeDelta < 80 ? "Drag Perfect" : "Drag Clear");
      ...
      function spawnNoteElement(state, note) {
        ...
        const rect = state.playfield.getBoundingClientRect();
        const startPoint = pickResponsivePoint(state, rect);
        note.x = startPoint.x;
        note.y = startPoint.y;
        if (note.type === "drag") {
          const endPoint = pickResponsivePoint(state, rect, [startPoint]);
          note.endX = endPoint.x;
          note.endY = endPoint.y;
        }
          ...
          state.chart.forEach((note) => {
            ...
            if (note.type === "hold") {
              ...
              if (trackTime >= note.endTimeMs - 110) {
                const delta = Math.abs(trackTime - note.endTimeMs);
                completeLaneNote(state, note, delta <= 90 ? 170 : 130, laneLabel(IS_ZH ? "长按" : "Hold", delta <= 90));
              }
              return;
            }

            if (note.type === "drag") {
              const expectedLane = note.lane + (note.endLane - note.lane) * clamp((trackTime - note.timeMs) / Math.max(1, note.endTimeMs - note.timeMs), 0, 1);
              ...
              if (trackTime >= note.endTimeMs - 120) {
                const delta = Math.abs(trackTime - note.endTimeMs);
                completeLaneNote(state, note, delta <= 90 ? 190 : 150, laneLabel(IS_ZH ? "拖拽" : "Drag", delta <= 90));
              }
            }
          });
        } else {
      ...
      async function startMusicSong(songKey, mode = "freeform") {
        if (!musicGameState) return;
        ...
        try {
          state.audio = new Audio(song.audioSrc);
          state.audio.preload = "auto";
          state.audio.volume = 1;
          const loaded = await loadSongChart(songKey, mode);
      ...
      function buildMusicGameUi() {
        ...
        shell.innerHTML = `
          <div class="music-game-header">
            <div class="music-game-actions">
              ...
              <button type="button" class="music-game-button music-game-select">${IS_ZH ? "选择歌曲" : "Select song"}</button>
              <button type="button" class="music-game-button music-game-exit">${IS_ZH ? "退出" : "Exit"}</button>
            </div>
            <div class="music-game-song-list">
              <button type="button" class="music-game-song-button" data-song="fiu" data-mode="freeform">fiu</button>
              <button type="button" class="music-game-song-button" data-song="fiu" data-mode="lanes">fiu lanes</button>
            </div>
          </div>
          <div class="music-game-hud">
            ...
            <div class="music-game-chip"><span>${IS_ZH ? "状态" : "Status"}</span><strong class="music-game-status-label">${IS_ZH ? "选择歌曲" : "Select a song"}</strong></div>
          </div>
        ...
        selectButton.addEventListener("click", () => {
          musicGameLayer.classList.toggle("is-song-list-open");
        });

        downloadButton.addEventListener("click", () => {
          preloadMusicGameData(musicGameState).catch(() => {});
        });
        ...
        songList.addEventListener("click", (event) => {
          ...
          if (!target) return;
          musicGameLayer.classList.remove("is-song-list-open");
          startMusicSong(target.getAttribute("data-song"), target.getAttribute("data-mode") || "freeform");
        });
