    ...
    (function () {
      ...
      function zhPrompt(text) {
        ...
        return text
          .replace(/^Choose the correct Spertz verb form for: /, "请选择正确的 Spertz 动词形式：")
          .replace(/^Choose the correct gloss for: /, "请选择正确释义：")
          .replace(/^Which subject suffix marks /, "哪个主语后缀标记 ")
          .replace(/^Type your answer...$/, "请输入答案…");
      }
      ...
      function gradeQuiz() {
        ...
        cards.forEach((card, idx) => {
          ...
          if (q.type === "mcq") {
            answer.textContent = IS_ZH ? `答案：${q.answer}` : `Answer: ${q.answer}`;
          } else {
      ...
      function calculate() {
        ...
        resultsBox.innerHTML = `
          <div class="personality-card">
            ...
            <div class="result-pills" style="margin-top:10px;">
              <span class="result-pill">${IS_ZH ? "戒环数" : "Rings"}: ${rings.length}</span>
            </div>
          </div>
          <div class="personality-card">
            ${rings.map((r, idx) => `
    ...
    (function () {
      ...
      function renderList(wrap, list, unlocked, namePrefix) {
        ...
        list.forEach((item, i) => {
          ...
          const span = document.createElement("span");
          span.textContent = i < unlocked ? item : cookLabel("Locked");
          label.appendChild(input);
          label.appendChild(span);
      ...
      function renderMethods() {
        ...
        METHODS.forEach((m, i) => {
          ...
          const span = document.createElement("span");
          span.textContent = i < STATE.unlockedMet ? m : cookLabel("Locked");
          label.appendChild(input);
          label.appendChild(span);
      ...
      function renderAll() {
        ...
        if (lastDishOut) lastDishOut.textContent = STATE.lastDish ? STATE.lastDish.name : cookLabel("None");
      ...
      function renderBook() {
        ...
        bookWrap.innerHTML = STATE.dishes.map((d, i) => {
          ...
          return `<div class="cook-book-entry ${priceCls} ${fancy}">
            ...
            <span>${cookLabel("Ingredients")}: ${d.ingredients.join(", ")}</span><br>
            <span>${cookLabel("Seasonings")}: ${d.seasonings.join(", ")}</span>
          </div>`;
        }).join("");
      }
      ...
      function renderHints() {
        ...
        hintWrap.innerHTML = STATE.hints.map((idx, i) => {
          ...
          return `<div class="cook-hint-item">
            <b>${i + 1}. ${r.name}</b><br>
            <span>${cookLabel("Method")}: ${r.method}</span><br>
            <span>${cookLabel("Ingredients")}: ${r.ingredients.join(", ")}</span><br>
            <span>${cookLabel("Seasonings")}: ${r.seasonings.join(", ")}</span>
          </div>`;
        }).join("");
      }
      ...
      function renderGuidebook() {
        ...
        guideWrap.innerHTML = CUSTOMER_TYPES.map((c) => {
          const likes = IS_ZH ? {
            fish: "必须包含 ember fish。",
            tofu: "必须包含 cloud tofu。",
            veg: "不含鱼；纯植物向。",
            child: "偏甜且不含 smoke pepper。",
            elder: "偏温和做法（simmer/steam），且不含 smoke pepper。",
            spicy: "需要 smoke pepper 或 chili flake。",
            sweet: "需要 sweet mist、star honey 或 night sugar。",
            herbal: "需要 herb spiral、vine leaf 或 mint dust。",
            sea: "需要 sea umami、mist shrimp 或 cobalt algae。",
            light: "steam/simmer 且食材不超过 2 种。",
            chef: "偏好草本或海味主导菜。",
            traveler: "偏好辛辣或甜味风格。",
            ritual: "偏好炖煮或蒸制菜。",
      ...
      function makeDish() {
        ...
        if (STATE.dishes.filter((d) => d.isAdvanced).length >= ADVANCED_RECIPES.length) {
          ...
          statusOut.textContent = coins
            ? (IS_ZH ? `已制作：${name}（+${coins} 金币）` : `Created: ${name} (+${coins} coins)`)
            : (IS_ZH ? `已制作：${name}（价格 ${price}）` : `Created: ${name} (price ${price})`);
        }
      }
  ...
  randomBirthBtn.addEventListener("click", randomBirthday);
    ...
    (function () {
      ...
      sorted.forEach(([sp, en], idx) => {
        const meaning = IS_ZH ? (ZH_MEANING[en] || en) : en;
