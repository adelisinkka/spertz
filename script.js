    ...
    (function () {
      ...
      const supportedLanguages = [
        "en", "zh-CN", "es", "fr", "ja", "ko", "de", "pt",
        "it", "ru", "ar", "hi", "id", "tr", "vi",
      ];
      const supported = new Set(supportedLanguages);
      let translateInitPromise = null;

      function normalizeLanguage(lang) {
        return supported.has(lang) ? lang : defaultLang;
      }

      function setDocumentLanguage(lang) {
        const htmlLang = lang === "zh-CN" ? "zh-Hans" : lang;
      ...
      function waitForGoogleCombo(maxChecks = 40) {
      ...
      async function applyLanguage(language) {
        ...
        try {
          ...
          const comboReady = await waitForGoogleCombo();
          if (comboReady) {
            setGoogleCombo(target);
          }
        } catch (error) {
          console.warn("Language switch failed; keeping English.", error);
          select.value = "en";
      ...
      const initialLanguage = normalizeLanguage(localStorage.getItem(storageKey) || defaultLang);
