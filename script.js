    ...
    (function () {
      ...
      function getLaneDragHand(note) {
        if (note.type !== "drag") return "";
        const startLane = note.lane ?? 0;
        const endLane = note.endLane ?? startLane;
        const averageLane = (startLane + endLane) * 0.5;
        return averageLane < (LANE_MODE_LANES - 1) * 0.5 ? "left" : "right";
      }
      ...
      function createLaneNoteElement(state, note) {
        ...
        const dragHand = getLaneDragHand(note);
