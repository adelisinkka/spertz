    ...
    (function () {
      ...
      function compressMelodyTimelineHard(melody, songKey = "") {
        ...
        melody.forEach((note, index) => {
          ...
          if (note[0] - lastTime < minGap && index % (songKey === "recordplayer" ? 2 : 3) !== 0) return;
      ...
      function buildHardLaneModeChart(songKey) {
        const builtin = BUILTIN_MUSIC_TIMINGS[songKey];
        if (!builtin) return null;
        const groupWindowMs = songKey === "recordplayer" ? 108 : 165;
        const crowdedWindowMs = songKey === "recordplayer" ? 112 : 185;
        const grouped = [];
      ...
      function advanceFreeformPath(state, note = null) {
        ...
        state.freeformPathCursor = ((current + step * (state.freeformPathDirection ?? 1)) % path.length + path.length) % path.length;
        state.freeformPatternStepCount = (state.freeformPatternStepCount || 0) + 1;
        if ((state.freeformPatternStepCount || 0) >= path.length) {
          state.freeformPatternStepCount = 0;
          state.freeformPathIndex = ((state.freeformPathIndex ?? 0) + 1) % FREEFORM_PATHS.length;
          state.freeformPathDirection = (state.freeformPathIndex ?? 0) % 2 === 0 ? 1 : -1;
          state.freeformPathCursor = state.freeformPathDirection > 0 ? 0 : (FREEFORM_PATHS[state.freeformPathIndex].length - 1);
        }
      }
      ...
      async function startMusicSong(songKey, mode = "freeform", difficulty = "normal") {
        ...
        state.freeformPathIndex = 0;
        state.freeformPathCursor = 0;
