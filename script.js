    ...
    (function () {
      ...
      function buildHardLaneModeChart(songKey) {
        ...
        function makePath(startLane, endLane, seed) {
          const pivot = seed % 2 === 0
            ? (startLane < endLane ? Math.max(0, startLane - 1) : Math.min(LANE_MODE_LANES - 1, startLane + 1))
            : (startLane < endLane ? Math.min(LANE_MODE_LANES - 1, endLane + 1) : Math.max(0, endLane - 1));
          const path = [startLane, pivot, endLane];
          if (Math.abs(endLane - pivot) <= 1 && seed % 3 === 0) {
            const bounce = endLane === 0 ? 1 : endLane === LANE_MODE_LANES - 1 ? LANE_MODE_LANES - 2 : (seed % 2 === 0 ? endLane - 1 : endLane + 1);
            path.push(bounce, endLane);
          }
          return path.filter((lane, index) => index === 0 || lane !== path[index - 1]);
        }
        ...
        function pushNote(timeMs, holdMs, lane, paired = false) {
          ...
          if (wantsDrag) {
            ...
            const pathLanes = makePath(lane, endLane, emitted);
            const finalLane = pathLanes[pathLanes.length - 1];
            const note = {
        ...
        grouped.forEach((group, groupIndex) => {
          ...
          const sorted = group.entries
            .slice()
            .sort((a, b) => ((b[1] > 0) - (a[1] > 0)) || (b[1] - a[1]) || (a[0] - b[0]));
          const targetCount = sorted.length > 1 || groupIndex % 4 === 0 ? 2 : 1;
      ...
      function getBuiltinLaneChart(songKey, difficulty = "normal") {
        ...
        return BUILTIN_LANE_MODE_CHARTS[songKey] || buildLaneModeChart(songKey) || null;
      }
